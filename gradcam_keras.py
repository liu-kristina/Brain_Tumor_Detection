...
def get_class_predictions(preds, im_no=0):
  decoded = ['glioma', 'meningioma', 'notumor', 'pituitary']
  prob = preds[im_no][np.argmax(preds)]
  label = "{}: {:.2f}%".format(decoded[np.argmax(preds)], prob * 100)
  print('Class with highest probability:')
  print("{}".format(label))

  return np.argmax(preds), decoded[np.argmax(preds)]